...
If your ByteString contains text using _only_ those characters from subset of Unicode covered by code points 0-255, i.e. Unicode Basic Latin, Latin-1 Supplement and C0+C1 Controls, encoded in UTF-8, then you can use the [`Data.ByteString.Char8`](https://hackage.haskell.org/package/bytestring-0.10.6.0/docs/Data-ByteString-Char8.html) package to work with the bytes in the string as if they were characters. This will work for English and some Western European languages, but will break as soon as any other character arises (for example, a smiley). 
...
Because `ByteString` funtions have the same name as normal `String` functions in the prelude, it needs to be imported with a qualification, e.g.

```
import qualified Data.ByteString as BS
```

However with the rise of emoticons, not even English speakers regularly use characters 0-255 anymore. For example this code snippet

```
BS.unpack $ BS.pack $ "H\x00C9llo \x1F4A9"`
```
is a _destructive_ operation which returns a different string to its input: `"H\x00C9llo \x00A9"`
...
So, the first thing we'll be doing is converting our `Location` module to use Text. We'll investigate the cost of this transition along the way, so the first thing to do is run `cabal bench` on your current implementaiotn and take a note of how long the `Show` implementation takes on average.
...
Lastly note that we have to explicitly call `T.unpack` to convert to convert to the desired `String` type. This is obviously wasteful, once the application becomes interactive we'll use the `Text` module's own `putStrLn` and avoid the conversion completely.
...
In the [next part of this tutorial](fixme) we'll address a problem of name-clashes and look into some useful Haskell extensions.
